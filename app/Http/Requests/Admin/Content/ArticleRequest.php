...
class ArticleRequest extends ContentRequest
{
    ...
    public function __construct()
    {
        ...
        $this->tableCategory = (new ArticleCategory)->getTable();
    }
    ...
    protected function storeRules(): array
    {
        ...
        return [
            ...
            'slug' => [
                'bail',
                ...explode('|', $this->stringRule(config('custom.length.article.slug_max'))),
            ...
            'title' => 'bail|'.$this->stringRule(config('custom.length.article.title_max'), true),
            'cover' => $this->arrayRule(),
            'cover.path' => $this->stringRule(config('custom.length.article.cover_max')),
            'cover.upload_id' => 'required_with:cover.path|'.$this->intRule().'|distinct|'.$this->existsRule($this->tableLog, 'id'),
            'cover_app' => $this->arrayRule(),
            'cover_app.path' => $this->stringRule(config('custom.length.article.cover_app_max')),
            'cover_app.upload_id' => 'required_with:cover_app.path|'.$this->intRule().'|distinct|'.$this->existsRule($this->tableLog, 'id'),
            'summary' => $this->stringRule(config('custom.length.article.summary_max')),
